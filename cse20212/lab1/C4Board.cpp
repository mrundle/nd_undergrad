...
/*
   Displays the current Connect Four board in simple text
*/
void C4Board::display(){
	...
	for(int i=board[0].getMaxDiscs()-1;i>=0;i--){
		...
		for(int j=0;j<numCols;j++){
			if(j<(numCols-1)) cout << board[j].getDisc(i) << "_|_";
			if(j==(numCols-1)) cout << board[j].getDisc(i) << "_|";
		}
		cout << "\n";
	}
...
/*
	...
	*** Since we are using dynamically defined arrary sizes, need to check each adjacent space to ensure that we
	    aren't trying to check beyond the scope of the array
*/
...
int C4Board::playerMove(int player){
	...
	while(1){
		...
		if(board[playerChoice].isFull()==0){
			if(player==1) board[playerChoice].addDisc('X');
			if(player==2) board[playerChoice].addDisc('O');
			return 1;
		}
	}
}
